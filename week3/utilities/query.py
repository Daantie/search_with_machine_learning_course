...
model = fasttext.load_model("/workspace/datasets/fasttext/query_classifier_0.5_5_2.bin")
...
def create_query(user_query, click_prior_query, sort="_score", sortDir="desc", size=10, source=None, categories=[],
                 should_boost_categories=False):
    filters = []
    category_boost = {}
    should_clauses = [
        {
    ...
    if not should_boost_categories:
        cat_filter = {
            "terms": {
...
def search(client, user_query, index="bbuy_products", sort="_score", sortDir="desc", should_boost_categories=False):
    ...
    query_obj = create_query(user_query, click_prior_query=None, sort=sort, sortDir=sortDir,
                             source=["name", "shortDescription"], categories=categories,
                             should_boost_categories=should_boost_categories)
...
if __name__ == "__main__":
    ...
    general.add_argument("--should_boost_categories", action=argparse.BooleanOptionalAction,
                         help="Whether to use categories for boosting or filtering")
    ...
    for line in sys.stdin:
        ...
        search(client=opensearch, user_query=query, index=index_name,
               should_boost_categories=args.should_boost_categories)
